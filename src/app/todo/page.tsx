  ...
  async function Filter() {
    try {
      const response = await fetch('http://localhost:8080/time/filter', {method : 'POST',
        headers: {"Content-Type": "application/json",},
        body: JSON.stringify({month : fetchMonthFilter}),
      })
      const data = await response.json();
      if(sortingState === "asc") { //토글 스위치하고 변수 상태가 맞지 않음, 정확히는 지금 상태의 sortingState를 비교하지 않고 전 상태를 비교함
        data.reverse(); //const를 쓰면 한번 밀리고, let이나 var를 쓰면 할때마다 다시 초기화가 선언되서 월로 넘어갈 때 CSS는 되지만, 변수가 유지가 안됨 -> 포기 ㅈㅈ
      }
      if(data.length === 0) {
        setDBIndex([]);
      }
